...
const checkTs = config => {
  if (config.typescript && !fs.existsSync(paths.appTsConfig)) {
    fs.writeFileSync(paths.appDirectory + '/tsconfig.json', fs.readFileSync(path.resolve(__dirname, '../../template/tsconfig.json'), 'utf-8'));
  }
};
...
module.exports = config => {
  ...
  return {
    ...
    module: {
      rules: [{
        ...
        use: {
          ...
          options: {
            presets: [require.resolve('@babel/preset-env'), require.resolve('@babel/preset-react')].concat(config.typescript ? require.resolve('@babel/preset-typescript') : [])
          }
        }
      }, {
        test: /\.css$/,
    ...
    plugins: [HTMLWebpackPluginConfig, MiniCssExtractPluginConfig].concat(config.typescript ? ForkTsCheckerWebpackPluginConfig : [])
  };
};